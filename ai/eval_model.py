...
def main():
    ...
    model = tf.keras.models.load_model('best_cancer_model_yet')

    # Load the metadata into a DataFrame
    test_folder_path = 'test_data/'
    test_image_path = test_folder_path + 'images/'
    csv_file = test_folder_path + 'test_data.csv'
    dataFrame = pd.read_csv(csv_file)
    model_image_size = [28, 28]

    # this changes the string labels into numbers. there are 7 labels so 0-6
    label_encoder = LabelEncoder()
    label_encoder.fit(dataFrame['dx'])
    dataFrame['label'] = label_encoder.transform(dataFrame["dx"])
    ...
    test_images_x = np.asarray(dataFrame['image'].tolist()).reshape(-1,28,28,3)
    test_images_x = test_images_x / 255.0

    # convert to categorical 
    test_labels_y = to_categorical(dataFrame['label'], num_classes=7)
    config_ = Configuration()
    image_gen = ImageDataGenerator(rescale=config_.generator_rescale)
    #image_gen.fit(test_images_x)
    testing_data_flow = image_gen.flow(test_images_x, test_labels_y, batch_size=100, shuffle=True)
